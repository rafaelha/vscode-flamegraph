...
export function parseProfilingData(data: string): ProfilingResults {
    ...
    lines.forEach((originalLine, lineIndex) => {
        ...
        frames.forEach((frame, frameIndex) => {
            ...
            // Skip if function name is enclosed by < and > and not <module>
            if (functionName.startsWith('<') && functionName.endsWith('>') && functionName !== '<module>') {
                console.log(`Skipping frame with function name: ${functionName}`);
                return;
            }
